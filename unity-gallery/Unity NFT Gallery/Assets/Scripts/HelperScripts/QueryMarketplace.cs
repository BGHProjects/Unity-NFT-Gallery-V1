...
public class QueryMarketplace
{
    ...
    public async Task<BigInteger> GetItemCounter()
    {
        ...
        var itemCounter = await queryHandler.QueryAsync<BigInteger>(BlockchainConstants.MARKETPLACE_ADDRESS, itemCounterMessage).ConfigureAwait(false);

        return itemCounter;
    }

    // Function to get all present Marketplace Items
    public async Task<MarketItemDTO> GetMarketItem(BigInteger itemId)
    {
        ...
        var marketItem = await queryHandler.QueryDeserializingToObjectAsync<MarketItemDTO>(marketItemsMessage, BlockchainConstants.MARKETPLACE_ADDRESS).ConfigureAwait(false);
